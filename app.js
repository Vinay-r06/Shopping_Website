...
// Cart with many to many relationship--sharp task..

// one-to-many and many-to-many relations--udem task..
...
//  Product.belongsTo(User, {constraints:true, onDelete: 'CASCADE'});                                                                                                                  
//  User.hasMany(Product);
//  User.hasOne(Cart);                     //key to the cart , new field to the cart--> userid
//  Cart.belongsTo(User);                 //optional
//  Cart.belongsToMany(Product, {through: CartItem})           // {}-->telling sequelize where these connection should be stored--"cart-item"
//  Product.belongsToMany(Cart, {through: CartItem})


// sequelize
// .sync()
// //.sync({force: true})                   // force:true--> recreate all table                                  
//           .then(result =>{                                                    
//          return  User.findByPk(1)
//             //console.log(result);
//         })
...
//         }); 
 ...
 //   lets work on cart
 // a cart belongto a user and 
 // a cart in turn holds products, many products with quantity associated to them..

//soo need more than one model
// now work cart model first...delete all code..

 //cart should belong to single user
// may hold multiple products..

// cart table should hold different carts for the different users...not just carts and model
// a new   cart-item.js
// each "cart" item is esstentially a combination of a "product" and the id of the "cart" in which this product lies and the quantity.id of the cart will added by sequelize by hasMany or something association...
// so now make association user and cart and import cart
// key to the cart , new field to the cart--> userid

// belongsToMany--> works with intermidate table connect them--> stores a combination of "product ID and Cart Id" for that i had "cart-item" model.

//  carts table is each user
// cart-item is many carts of users in one organization or company..




// creating and fetching  a cart..task --udem


// lets work oon cart
// go to shop.js in  controller 
// their change the code--> ".getCart" 
// want use cart associated with my exiting user to get all the products in it
// and diasable force: true in sunc sequelize
// now create cart in app-->.createCart()
// we loaded cart from the database..
// we can use cart by fetch the products...and render the view..
// run and see the cart page...it will come "no products"..





// adding new products to the cart----task-udem...
...
// to add cart--work on postcart method...in "shop.js" in controller
// run and check-->add product and submit-->browser will get error...check in sql cartitem table.. u will see ...   
...
// adding exiting products and retrieving cart items...---task-udem...
...
//         }); 
        ...
        // lets increase the quantity now
        // make some changes in cart.ejs in views-->shop-->cart.ejs
        // then increase quantity code in shop.js
        // take that product from cart with quantity value and increase by one...
// run and check by add product and check in cart page and sql table..
...
// deleting related items and deleting cart products------task---udem
...
//    change the code in postCartDeleteProduct
// run and check ---delete product in cart and check in cartItem table it will be deleted...